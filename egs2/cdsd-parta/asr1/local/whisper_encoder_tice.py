...
class FactorizedLoraLinear(nn.Module):
    def __init__(self, original_linear: nn.Linear, r: int = 4, lora_alpha: int = 16, lora_dropout: float = 0.01):
        super().__init__()
        ...
        self.A1 = nn.Parameter(torch.zeros(r, d_in))
        self.B1 = nn.Parameter(torch.zeros(d_out, r))
        self.A2 = nn.Parameter(torch.zeros(r, d_in))
        self.B2 = nn.Parameter(torch.zeros(d_out, r))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h1 = F.linear(x_drop, self.A1)
        side_out1 = F.linear(h1, self.B1) * self.a1 * self.scaling
        h2 = F.linear(x_drop, self.A2)
        side_out2 = F.linear(h2, self.B2) * self.a2 * self.scaling
        
        return base_out + side_out1 + side_out2
...
class OpenAIWhisperEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        for name, module in self.encoders.named_modules():
            if isinstance(module, torch.nn.Linear) and module.__class__.__name__ == "Linear":
                if not any(k in name for k in ["output_layer", "lm_head"]):
                    if any(target in name for target in ["query", "key", "value", "out", "mlp.0", "mlp.2"]):
                        modules_to_replace.append((name, module))

        if len(modules_to_replace) == 0:
            logging.error("❌ ERROR: modules_to_replace is EMPTY in Encoder!")
            raise ValueError("No target layers found to replace in Encoder.")

        for name, module in modules_to_replace:
            labels = name.split('.')
            submodule = self.encoders
        ...
        for name, sub_module in self.encoders.named_modules():
            if sub_module.__class__.__name__ == "FactorizedLoraLinear":
                for param in sub_module.parameters():
                    if id(param) != id(sub_module.original_linear.weight) and (sub_module.original_linear.bias is None or id(param) != id(sub_module.original_linear.bias)):
                        param.requires_grad = True
            # if "ln" in name or "layer_norm" in name:
            #     for param in sub_module.parameters():
            #         param.requires_grad = True
        trainable_params = [n for n, p in self.encoders.named_parameters() if p.requires_grad]
        logging.info(f"============ [Encoder Side-Branch Check] ============")
        logging.info(f"Total trainable parameter keys in Encoder: {len(trainable_params)}")
        for name in trainable_params[:20]:
            logging.info(f" -> Trainable Encoder Part: {name}")
        # ========================================================添加结束
        ...
        with torch.no_grad():
            for name, module in self.encoders.named_modules():
                if module.__class__.__name__ == "FactorizedLoraLinear":
                    ...
                    for label in labels[:-1]:
                        submodule = getattr(submodule, label)
                    setattr(submodule, labels[-1], module.original_linear)
